...
@implementation LoadHooker
+ (void)load {
    NSLog(@"loadHooder 执行load方法了");
        ...
        if ([imagePath containsString:bundlePath] && ![imagePath containsString:@".dylib"]) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (![className isEqualToString:@""] && className) {
                    ...
                    Method swizzledMethod = class_getClassMethod([LoadHooker class], swizzledSelector);
...
+ (void)LDAPM_Load {
    ...
    NSLog(@"%@ load方法耗时 %f",NSStringFromClass([self class]), end - start);
